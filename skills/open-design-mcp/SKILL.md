...
`open-design-mcp` exposes **8 tools** that fall into 4 workflows:
...
| **Generate** | `od_generate_design`, `od_save_artifact` | Streams an HTML design from a PRD using a Bring-Your-Own-Key LLM, then persists it. |
...
| "Save this HTML" | "persist", "save artifact", "store this design" | `od_save_artifact` (after lint) |
...
| `od_save_artifact` | Persist an HTML artifact to the daemon under a slug identifier. Requires only `OD_DAEMON_URL`. | `OD_DAEMON_URL` |
...
4. od_save_artifact {              # persists under slug
     projectId,
     slug: "pricing-v1",           # human-readable, kebab-case
     html
   }
```
...
od_save_artifact { projectId, slug, html }   # overwrites existing slug
```

`od_save_artifact` is upsert-by-slug — same slug overwrites.
