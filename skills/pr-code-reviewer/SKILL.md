...
compatibility: "OpenCode with nano-brain CLI"
metadata:
  author: Sisyphus
  version: "3.4.0"
...
### GitHub CLI (Required for PR Reviews)
...
### Local Reviews (No gh CLI Required)
...
2. **If worktree found**, read its checkpoint manifest:
   ```bash
...
3. **Validate checkpoint is still valid**:
   ...
   - If SHA mismatch → checkpoint is stale, call `deleteworktree(branch="${head_branch}")` and start fresh
...
6. **If user says no or checkpoint invalid**:
   - Call `deleteworktree(branch="${head_branch}")` to remove existing worktree
...
- Cleanup is clean and tracked (no orphaned `/tmp` directories)

**Steps:**

1. **Extract repo info** from PR URL or `gh pr view`:
   - `repo` — repository name only (e.g., `tradeit-backend`)
   - `pr_number` (e.g., `1159`)
   - `head_branch` (e.g., `feature/dev-4650-callback-url-for-payment-method`)

2. **Create a worktree** using the `createworktree` tool:
   ```
   createworktree(
     branch: "${head_branch}",
   ...
   ```
   - If `createworktree` returns `{ ok: false }` → STOP review, report error to user.
   - Store the returned `worktreePath` as `$WORKTREE_PATH`.

3. **Verify worktree succeeded**:
   ```bash
   git -C "$WORKTREE_PATH" log --oneline -1
   ```
   If this fails → call `deleteworktree(branch="${head_branch}")` then STOP, report error.

4. **Record the worktree path** — ALL subsequent phases use `$WORKTREE_PATH` for file reads, grep, LSP, etc.
...
1. `gh pr view <N> -R {owner}/{repo} --json title,body,author,baseRefName,headRefName,additions,deletions,changedFiles` → title, description, author, base branch
2. `gh pr diff <N> -R {owner}/{repo} --name-only` → changed file list; `gh pr diff <N> -R {owner}/{repo}` → full diff
3. Read full file context from `$WORKTREE_PATH` (NOT from GitHub API — local reads are faster and give full file)
